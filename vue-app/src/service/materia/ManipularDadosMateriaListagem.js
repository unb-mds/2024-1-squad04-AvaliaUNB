...
export async function obterMaterias() {
...
function CalcularPesoCriteriosAvaliacaoSemFiltro(medias) {
  const { nota_total, nota_experiencia, nota_dificuldade } = medias;
  ...
  if (nota_total !== 0) {
    ...
    if (SomaMedidas !== 0) {
      ContribuicaoExperiencia = Math.round(
        (nota_experiencia / SomaMedidas) * 100
      );
      ContribuicaoDificuldade = Math.round(
        (nota_dificuldade / SomaMedidas) * 100
      );
    }
